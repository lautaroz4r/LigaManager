...
def seleccionar_equipo(entrada):
    entrada = entrada.strip()

    if entrada.isdigit():
        indice = int(entrada) - 1

        if indice >= 0 and indice < len(equipos):
            return equipos[indice]
        else:
            return None

    return buscar_equipo(entrada)
...
def cargar_resultado():
    ...
    entrada_local = input("\nIngrese el nombre o número del equipo local: ").strip()
    entrada_visitante = input("Ingrese el nombre o número del equipo visitante: ").strip()

    equipo_local = seleccionar_equipo(entrada_local)
    equipo_visitante = seleccionar_equipo(entrada_visitante)
